...
class StructuredTrainer:
    ...
    def _monitor_encoder_specialization(
        self, 
        enc_idx: int, 
        mus: chex.Array, 
        logvars: chex.Array, 
        ...
        """
        Monitor encoder specialization progress during training.
        
        This method tracks how well each encoder is specializing on its target pattern:
        - Target pattern: should have LOW variance (high certainty)
        - Other patterns: should have HIGH variance (low certainty)
        
        Args:
            enc_idx: Index of the encoder being monitored
            mus: (B, N, H) - means from this encoder
            logvars: (B, N, H) - log variances from this encoder
            pattern_ids: (B,) - pattern IDs for each sample
            step: Current training step
            
        Returns:
            dict: Specialization metrics for monitoring
        """
        try:
            # Convert JAX arrays to numpy for analysis
            mus_np = np.array(mus)
            logvars_np = np.array(logvars)
            pattern_ids_np = np.array(pattern_ids)
            
            # Target pattern for this encoder
            target_pattern = enc_idx + 1  # Encoder 0 → Pattern 1, Encoder 1 → Pattern 2, etc.
            
            # Compute variances
            variances = np.exp(logvars_np)  # (B, N, H)
            
            # Average variance per sample (over pairs and latent dimensions)
            sample_variances = np.mean(variances, axis=(-2, -1))  # (B,)
            
            # Separate variances by pattern
            target_mask = (pattern_ids_np == target_pattern)
            other_mask = ~target_mask
            
            if np.any(target_mask) and np.any(other_mask):
                target_variances = sample_variances[target_mask]
                other_variances = sample_variances[other_mask]
                
                # Compute specialization metrics
                target_mean_var = np.mean(target_variances)
                other_mean_var = np.mean(other_variances)
                
                # Specialization ratio: target variance should be LOWER than other variance
                specialization_ratio = target_mean_var / (other_mean_var + 1e-8)
                
                # Specialization score: closer to 0 means better specialization
                # (target variance << other variance)
                specialization_score = np.log(specialization_ratio + 1e-8)
                
                # Variance difference: should be negative (target < other)
                variance_diff = target_mean_var - other_mean_var
                
                # Log specialization progress
                if step % 100 == 0:  # Log every 100 steps to avoid spam
                    logging.info(f"Encoder {enc_idx} Specialization Progress (Step {step}):")
                    logging.info(f"  Target Pattern {target_pattern}: mean variance = {target_mean_var:.6f}")
                    logging.info(f"  Other Patterns: mean variance = {other_mean_var:.6f}")
                    logging.info(f"  Specialization ratio: {specialization_ratio:.3f} (target/other)")
                    logging.info(f"  Specialization score: {specialization_score:.3f} (log ratio)")
                    logging.info(f"  Variance difference: {variance_diff:.6f} (target - other)")
                ...
                return {
                    f"encoder_{enc_idx}/specialization/target_pattern_mean_variance": target_mean_var,
                    f"encoder_{enc_idx}/specialization/other_patterns_mean_variance": other_mean_var,
                    f"encoder_{enc_idx}/specialization/specialization_ratio": specialization_ratio,
                    f"encoder_{enc_idx}/specialization/specialization_score": specialization_score,
                    f"encoder_{enc_idx}/specialization/variance_difference": variance_diff,
                    f"encoder_{enc_idx}/specialization/target_samples_count": int(np.sum(target_mask)),
                    f"encoder_{enc_idx}/specialization/other_samples_count": int(np.sum(other_mask)),
                }
            else:
                logging.warning(f"Encoder {enc_idx}: Cannot compute specialization - missing pattern diversity")
                return {}
