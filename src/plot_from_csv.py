...
# Helper function to find the first budget where a method achieves accuracy = 1
def find_first_accuracy_one(data_by_metric: Dict, method: str, metric: str, steps: List[int], budgets: List[int]) -> Optional[int]:
    """
    Find the first budget where a method achieves accuracy = 1.
    
    Args:
        data_by_metric: Dictionary containing the data organized by metric, method, step, and budget
    ...
    Returns:
        First budget value where accuracy = 1, or None if never achieved
    """
    if method not in data_by_metric.get(metric, {}):
        return None
    
    method_data = data_by_metric[metric][method]
    
    # For dataset_length = 1 case, check if any step achieves accuracy = 1
    # For dataset_length > 1 case, check if mean accuracy across steps = 1
    
    # Check if we have multiple steps (dataset_length > 1)
    if len(steps) > 1:
        # Calculate mean accuracy across all steps for each budget
        for budget in sorted(budgets):
            ...
            if accuracies:
                ...
                if mean_acc >= 1.0:  # Use >= to handle floating point precision
                    return budget
    else:
        # Single step case - check if any budget achieves accuracy = 1
        for budget in sorted(budgets):
            if steps[0] in method_data and budget in method_data[steps[0]]:
                acc = method_data[steps[0]][budget]
...
def add_accuracy_one_markers(ax, data_by_metric: Dict, methods: List[str], metric: str, 
                            steps: List[int], budgets: List[int], budget_to_index: Dict[int, int]):
    """
    Add dashed horizontal lines where each method achieves accuracy = 1.
    
    Args:
        ax: Matplotlib axis to add markers to
        data_by_metric: Dictionary containing the data
        methods: List of method names
        metric: Metric name to check
        steps: List of training steps
        budgets: List of budget values
        budget_to_index: Mapping from budget values to array indices
    """
    # Define colors for each method
    method_colors = {
        'evolutionary_search': 'purple',  # ES - purple
        'gradient_ascent': 'orange'       # GA - orange
    }
    
    for method in methods:
        first_accuracy_one_budget = find_first_accuracy_one(data_by_metric, method, metric, steps, budgets)
        
        if first_accuracy_one_budget is not None:
            # Get the index for this budget
            if first_accuracy_one_budget in budget_to_index:
                budget_index = budget_to_index[first_accuracy_one_budget]
                
                # Get the color for this method
                color = method_colors.get(method.lower(), 'gray')
                
                # Add dashed horizontal line (since y-axis is budget)
                ax.axhline(y=budget_index, color=color, linestyle='--', linewidth=2, alpha=0.8,
                          label=f'{method.replace("_", " ").title()} achieves {metric.replace("_", " ").title()} = 1')
                
                # Add text annotation
                ax.text(0.02, budget_index + 0.02, f'{method.replace("_", " ").title()}: {metric.replace("_", " ").title()} = 1',
                       transform=ax.transData, color=color, fontweight='bold', fontsize=10,
