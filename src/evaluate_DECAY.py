...
def run_evolutionary_search_single(
    ...
    dataset_seed: int = 0,
    out_dir: str = "results/es_decay_budget_sweep",
    ...
    run_out_dir = os.path.join(out_dir, f"es_decay_{decay_rate:.6f}_budget_{max_budget}")
...
def create_heatmap(
    ...
    ax.set_yticks(list(decay_rates))
    ax.set_yticklabels([f"{v:.3f}" for v in decay_rates])
    ...
    fig_path = out_dir / "es_decay_budget_heatmap.png"
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate evolutionary search with different mutation standard deviation decay rates and budgets")
    ...
    # W&B logging
    parser.add_argument("--wandb_project", type=str, default="ES_DECAY_SWEEP", help="Weights & Biases project to log results")
    ...
    print(f"🔬 Decay Rate (Mutation Std Decay) Sweep Configuration:")
    print(f"   - Start: {args.decay_start}")
    print(f"   - End: {args.decay_end}")
    print(f"   - Steps: {args.decay_steps}")
    print(f"   - Values: {decay_rates}")
    ...
    out_dir = Path("results/es_decay_budget_sweep")
    out_dir.mkdir(parents=True, exist_ok=True)
    
    # Results matrices: [decay_idx, budget_idx] -> metrics (budget-aligned)
    results_matrix = np.full((len(decay_rates), len(budgets)), np.nan)  # Loss matrix
    accuracy_matrix = np.full((len(decay_rates), len(budgets)), np.nan)  # Accuracy matrix
    shape_accuracy_matrix = np.full((len(decay_rates), len(budgets)), np.nan)  # Shape accuracy matrix
    pixel_correctness_matrix = np.full((len(decay_rates), len(budgets)), np.nan)  # Pixel correctness matrix
    execution_times = np.full((len(decay_rates), len(budgets)), np.nan)

    # CSV logging
    timestamp = time.strftime("%Y%m%d_%H%M%S")
    csv_path = out_dir / f"es_decay_budget_sweep_{args.run_name}_{timestamp}.csv"
    
    # Initialize W&B run
    run = None
    if not args.no_wandb and _WANDB_AVAILABLE:
        run_name = f"es_decay_sweep_{args.run_name}_{timestamp}"
        try:
    ...
    with open(csv_path, 'w', newline='') as f_csv:
        ...
        writer.writerow([
            "timestamp", "run_name", "checkpoint_name", "checkpoint_step", "decay_rate", "budget", 
        ...
        for i, decay_rate in enumerate(decay_rates):
            print(f"\n🔬 Testing decay_rate = {decay_rate:.6f} ({i+1}/{len(decay_rates)})")
            ...
            if intermediate_losses:
                ...
                if success:
                    successful_evals += 1
                    print(f"✅ Success: decay_rate={decay_rate:.6f}, extracted {len(intermediate_losses)} budget points, time={exec_time:.2f}s")
                else:
                    print(f"⚠️  Partial success: decay_rate={decay_rate:.6f}, extracted {len(intermediate_losses)} budget points, time={exec_time:.2f}s (evaluation failed but losses extracted)")
            else:
                failed_evals += 1
                print(f"❌ Failed: decay_rate={decay_rate:.6f} (no loss data available)")
    ...
    # Forward-fill missing cells per decay rate to avoid empty spots (e.g., budgets below first available)
    try:
        matrices_to_fill = [
            (results_matrix, "loss"),
    ...
    if successful_evals > 0:
        ...
        heatmap_path, fig = create_heatmap(decay_rates, budgets, results_matrix, str(out_dir))
    ...
    # Summary
    print("\n" + "=" * 60)
    print("📈 EVOLUTIONARY SEARCH DECAY RATE vs BUDGET SWEEP SUMMARY")
    print("=" * 60)
    ...
    print(f"Total evaluations: {len(decay_rates)} (one per decay rate with intermediate budget extraction)")
    print(f"Decay rate range: {args.decay_start} to {args.decay_end}")
    ...
    if successful_evals > 0:
        ...
        best_decay_idx, best_budget_idx = np.unravel_index(best_idx, results_matrix.shape)
        best_decay_rate = decay_rates[best_decay_idx]
        best_budget = budgets[best_budget_idx]
        best_loss = results_matrix[best_decay_idx, best_budget_idx]
        print(f"\n🏆 Best configuration:")
        print(f"   Decay Rate (Mutation Std Decay): {best_decay_rate:.6f}")
        print(f"   Budget: {best_budget}")
        print(f"   Total Loss: {best_loss:.6f}")
        
        # Show best accuracy metrics if available
        best_accuracy = accuracy_matrix[best_decay_idx, best_budget_idx]
        best_shape_accuracy = shape_accuracy_matrix[best_decay_idx, best_budget_idx]
        best_pixel_correctness = pixel_correctness_matrix[best_decay_idx, best_budget_idx]
        ...
        if run is not None:
            try:
                log_data = {
                    "best/decay_rate": float(best_decay_rate),
    ...
    if run is not None:
        try:
            art = wandb.Artifact(name=f"es_decay_budget_sweep_{args.run_name}_{timestamp}", type="es_decay_sweep")
