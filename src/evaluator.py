...
from  datasets.task_gen.re_arc_generators import ARC_TASK_NAMES
...
class Evaluator:
    ...
    def track_search_progress(self, trajectory_info):
        """Track the search optimization progress"""
        return {
            'losses': trajectory_info.get('losses', []),
            'accuracies': trajectory_info.get('accuracies', []),
            'latent_norms': trajectory_info.get('latent_norms', []),
            'step_times': trajectory_info.get('step_times', [])
        }
