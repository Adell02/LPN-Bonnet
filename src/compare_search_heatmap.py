...
def create_heatmaps(
    ...
    Args:
        ...
        cumulative: If True, each checkpoint includes data from all previous checkpoints
    """
    
    heatmap_files = []
    
    # Aggregate data across all checkpoints to create proper 2D heatmaps
    all_checkpoints = []
    all_ga_losses = []
    all_es_losses = []
    all_ga_budgets = []
    ...
    if cumulative:
        print(f"🔄 Using cumulative aggregation: each checkpoint includes all previous checkpoints")
    else:
        print(f"🔄 Using individual checkpoint data")
    
    for i, checkpoint_data in enumerate(checkpoint_results):
        checkpoint_name = checkpoint_data["checkpoint_name"]
        
        if cumulative:
            # For cumulative aggregation, include data from all checkpoints up to current one
            print(f"Processing checkpoint {i+1} with cumulative data (checkpoints 0-{i})")
            
            # Aggregate data from all checkpoints up to current one
            cumulative_ga_losses = []
            cumulative_es_losses = []
            cumulative_ga_budgets = []
            cumulative_es_budgets = []
            
            for j in range(i + 1):  # Include all previous checkpoints
                prev_checkpoint = checkpoint_results[j]
                prev_ga_results = prev_checkpoint["ga_results"]
                prev_es_results = prev_checkpoint["es_results"]
                
                # Extract losses and budget trajectories for both methods
                if max_budget in prev_ga_results and "losses" in prev_ga_results[max_budget]:
                    ga_losses = prev_ga_results[max_budget]["losses"]
                    ga_budget = prev_ga_results[max_budget].get("budget", np.arange(len(ga_losses)) * 2)
                    cumulative_ga_losses.append(ga_losses)
                    cumulative_ga_budgets.append(ga_budget)
                    
                if max_budget in prev_es_results and "losses" in prev_es_results[max_budget]:
                    es_losses = prev_es_results[max_budget]["losses"]
                    es_budget = prev_es_results[max_budget].get("budget", np.arange(len(es_losses)) * 4)
                    cumulative_es_losses.append(es_losses)
                    cumulative_es_budgets.append(es_budget)
            
            # Combine all cumulative data
            if cumulative_ga_losses:
                combined_ga_losses = np.concatenate(cumulative_ga_losses, axis=0)
                combined_ga_budgets = np.concatenate(cumulative_ga_budgets, axis=0)
                print(f"Cumulative GA: {len(cumulative_ga_losses)} checkpoints, total shape: {combined_ga_losses.shape}")
            else:
                combined_ga_losses = None
                combined_ga_budgets = None
                
            if cumulative_es_losses:
                combined_es_losses = np.concatenate(cumulative_es_losses, axis=0)
                combined_es_budgets = np.concatenate(cumulative_es_budgets, axis=0)
                print(f"Cumulative ES: {len(cumulative_es_losses)} checkpoints, total shape: {combined_es_losses.shape}")
            else:
                combined_es_losses = None
                combined_es_budgets = None
                
            # Store cumulative data
            all_checkpoints.append(f"{checkpoint_name}_cumulative_0_to_{i}")
            if combined_ga_losses is not None:
                all_ga_losses.append(combined_ga_losses)
                all_ga_budgets.append(combined_ga_budgets)
            if combined_es_losses is not None:
                all_es_losses.append(combined_es_losses)
                all_es_budgets.append(combined_es_budgets)
        else:
            # For individual checkpoint processing (original behavior)
...
def main():
    ...
    wandb.init(
        ...
        entity=args.entity,
        name=f"heatmap_comparison_{args.run_name}_{aggregation_type}",
        config=vars(args)
    )
    
    print(f"🔄 Aggregation Mode: {'Cumulative' if args.cumulative else 'Individual'}")
    if args.cumulative:
        print("   - Each checkpoint will include data from all previous checkpoints")
        print("   - Checkpoint 0: Single checkpoint data")
        print("   - Checkpoint 1: Data from checkpoints 0-1")
        print("   - Checkpoint 2: Data from checkpoints 0-2")
        print("   - And so on...")
    else:
        print("   - Each checkpoint will use only its own data")
        ...
        for i, checkpoint_path in enumerate(checkpoints_to_evaluate):
            ...
            if ga_results or es_results:
                ...
                heatmap_files = create_heatmaps(
                    checkpoint_results, max_budget, args.output_dir, args.cumulative
                )
    ...
    if checkpoint_results:
        ...
        summary_heatmaps = create_heatmaps(checkpoint_results, max_budget, args.output_dir, args.cumulative)
